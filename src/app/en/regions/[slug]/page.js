...
export default async function RegionDetailPage({ params }) {
    ...
    return (
        <div className={styles.pageContainer}>
            <BannerSection
                imageUrl={region.data.cover_photo}
                alt={`Cover photo of ${region.data.name}`}
            />
            <Breadcrumb
                items={[
                    { label: "Home", href: "/", icon: faHome },
                    { label: region.data.slug, active: true }
                ]}
            />
            <div className={styles.container}>
                <div className={styles.mainContent}>
                    <h1 className="text-center mb-4 text-dark">{region.data.name}</h1>
                   <div className={styles.regionDescriptionWrapper}>
    <p
        className={styles.regionDescription}
        dangerouslySetInnerHTML={{ __html: region.data.description || "" }}
    />
</div>

  {region.data.carousel && region.data.carousel.length > 0 && (
    <section className={styles.gallerySection}>
        <h2 className={styles.galleryTitle}>Gallery</h2>
        <div className={styles.galleryGrid}>
            {region.data.carousel.map((carousel, index) => (
                <div key={index} className={styles.galleryCard}>
                    ...
                    {region.data.destinations && region.data.destinations.length > 0 && (
                        <section className="mb-5">
                            <h2 className="border-bottom pb-2 mb-4 text-dark">Popular Destinations</h2>
                            <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 justify-content-center">
                                {region.data.destinations.map((destination, index) => {
                    ...
                    {region.data.events && region.data.events.length > 0 && (
                        <section className="mb-5">
                            <h2 className="border-bottom pb-2 mb-4 text-dark">Events</h2>
                            <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 justify-content-center">
                                {region.data.events.map((event, index) => {
                                    const imgSrc = "https://www.go-myanmar.com/files/destination-photo/2_hindu_temple_on_myitkyina_riverfront.jpg";
                                    return (
                                        <div key={index} className="col">
                                            <div className={`card h-100 shadow-lg border-0 ${styles.destinationCard}`}>
