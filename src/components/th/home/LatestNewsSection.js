...
if (!loading && !error && newsList.length === 0) {
  ...
  return (
    <section className={styles.latestNewsSection}>
      <div className="container py-5">
        <h2 className="text-center mb-5" style={{ fontSize: "2rem" }}>ข่าวล่าสุด</h2>
        {loading && (
  ...
  ></p>
                    <Link href={`/th/news/${news.slug}`} className={styles.readMore}>
                      อ่านเพิ่มเติม
                    </Link>
                  </div>
                </div>
        ...
        <div className="text-center mt-4">
          <Link href="/th/news" className="btn btn-outline-primary rounded-0">
            ดูข่าวทั้งหมด
          </Link>
        </div>
      </div>
