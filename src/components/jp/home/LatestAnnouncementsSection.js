...
import { fetchAnnouncementList } from "@/lib/api/jp-site/announcement";
...
const truncateDescription = (description, maxLength) => {
  ...
  {new Date(item.published_at).toLocaleDateString("en-US", {
    ...
    ></p>
                    <Link href={`/jp/announcements/${item.slug}`} className={styles.readMoreBtn}>
                    詳細を見る
                    </Link>
                  </div>
                </div>
        ...
        <div className={styles.viewAllWrapper}>
          <Link href="/jp/announcements" className={styles.viewAllBtn}>
            すべてのお知らせを見る
          </Link>
        </div>
      </div>
