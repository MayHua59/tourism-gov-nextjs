...
if (!loading && !error && newsList.length === 0) {
  ...
  return (
    <section className={styles.latestNewsSection}>
      <div className="container py-5">
        <h2 className="text-center mb-5" style={{ fontSize: "2rem" }}>Neueste Nachrichten</h2>
        {loading && (
  ...
  ></p>
                    <Link href={`/de/news/${news.slug}`} className={readMore}>
                      Mehr erfahren
                    </Link>
                  </div>
                </div>
        ...
        <div className="text-center mt-4">
          <Link href="/de/news" className="btn btn-outline-primary rounded-0">
